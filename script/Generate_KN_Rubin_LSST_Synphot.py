...
#     process_spectrum(nn, specfile, rep_wmetatbl, PHOT_WOLLAEGER_RUBIN_DATA)
...
for nn, (specfile, typ) in enumerate(zip(spectra, types)):
	...
	process_spectrum(nn, specfile, typ, PHOT_WOLLAEGER_RUBIN_DATA)

tmptables = sorted(glob.glob(f"{PHOT_WOLLAEGER_RUBIN_DATA}/*.tmp"))
...
synphot_anomaly_class_output_table = os.path.join(PHOT_WOLLAEGER_RUBIN_DATA, "synphot_anomaly_class.csv")	
...
logtxt += "END\n"
logtxt_path = os.path.join(PHOT_WOLLAEGER_RUBIN_DATA, 'log.txt')
