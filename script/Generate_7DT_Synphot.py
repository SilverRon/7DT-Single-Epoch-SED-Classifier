...
for ss, (specfile, typ) in enumerate(zip(spectra, types)):
    ...
    process_synphot(ss, specfile, typ, PHOT_7DT_DATA, sdt, fill_nan_in_array, flamunit)

tmptables = sorted(glob.glob(f"{PHOT_7DT_DATA}/*.tmp"))
...
synphot_normal_class_output_table = os.path.join(PHOT_7DT_DATA, "synphot_normal_class.csv")	
...
logtxt += "END\n"
logtxt_path = os.path.join(PHOT_7DT_DATA, "log.txt")
