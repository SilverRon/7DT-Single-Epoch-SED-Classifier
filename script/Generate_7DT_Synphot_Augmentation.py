...
# flux_scaleing_factors = [1e-2, 1e-1, 1e0, 1e1, 1e2] # 1e0 for the original one
flux_scaleing_factors = [1e-1, 1e0, 1e1] # 1e0 for the original one
number_of_iterations = 5

expected_total_number_of_samples = len(spectra) * len(flux_scaleing_factors) * number_of_iterations
print(f"Expected total number of samples: {expected_total_number_of_samples}")
...
for ss, (specfile, typ) in enumerate(zip(spectra, types)):
    ...
    process_synphot_augmentation(ss, specfile, typ, AUGMNETED_PHOT_7DT_DATA, sdt, number_of_iterations, flux_scaleing_factors)
    # break

tmptables = sorted(glob.glob(f"{AUGMNETED_PHOT_7DT_DATA}/*.tmp"))
...
synphot_normal_class_output_table = os.path.join(AUGMNETED_PHOT_7DT_DATA, "synphot_normal_class.csv")	
...
logtxt += "END\n"
logtxt_path = os.path.join(AUGMNETED_PHOT_7DT_DATA, "log.txt")
