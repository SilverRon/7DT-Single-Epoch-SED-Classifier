...
for ss, (specfile, typ) in enumerate(zip(spectra, types)):
    ...
    process_synphot(ss, specfile, typ, PHOT_WOLLAEGER_DATA, sdt, fill_nan_in_array, flamunit)
    # break

tmptables = sorted(glob.glob(f"{PHOT_WOLLAEGER_DATA}/*.tmp"))
...
synphot_anomaly_class_output_table = os.path.join(PHOT_WOLLAEGER_DATA, "synphot_anomaly_class.csv")	
...
logtxt += "END\n"
logtxt_path = os.path.join(PHOT_WOLLAEGER_DATA, "log.txt")
