...
def extrapolate_with_trend(lam, flam, filter_lam_min, filter_lam_max,
    ...
    if lammin > filter_lam_min:
        # Create blue wavelengths from filter_lam_min up to just below lammin
        bluelamarr = np.arange(filter_lam_min, lammin, median_res)
        ...
        blueflamarr = f_start + df_median * np.arange(-len(bluelamarr), 0)
    ...
    if lammax < filter_lam_max:
        # Create red wavelengths from just above lammax up to filter_lam_max
        redlamarr = np.arange(lammax + median_res, filter_lam_max + median_res, median_res)
        redlamarr = redlamarr[redlamarr <= filter_lam_max]
        df = np.diff(flam[-edge_width:])
        df_median = np.median(df)
        f_start = np.median(flam[-median_base:])
        redflamarr = f_start + df_median * np.arange(1, len(redlamarr) + 1)
    ...
    # Combine and deduplicate wavelengths
    interp_lam = np.array(interp_lam)
    interp_flam = np.array(interp_flam)
    unique_idx = np.argsort(interp_lam)
    interp_lam = interp_lam[unique_idx]
    interp_flam = interp_flam[unique_idx]
    # Remove exact duplicates
    lam_u, idx_first = np.unique(interp_lam, return_index=True)
    interp_lam = lam_u
    interp_flam = interp_flam[idx_first]
...
## Augmentation settings are now determined dynamically per spectrum below.

def process_spectrum_augmentation(nn, specfile, typ, path_save, number_of_iterations, flux_scaling_factors):
    """
    Process a spectrum file with multiple flux scaling augmentations,
    aggregating all results into one output file.
    """
    basename = os.path.splitext(os.path.basename(specfile))[0]
    table_path = os.path.join(path_save, f"{nn:0>4}.tmp")
    ...
    # Read and preprocess spectrum
    sptbl = Table.read(specfile, format='ascii')
    keys = sptbl.keys()
    lam, flam = process_flux(sptbl[keys[0]], sptbl[keys[1]])

    # Prepare to collect augmented magnitude rows
    n_augmented = number_of_iterations * len(flux_scaling_factors)
    # Preallocate output table with appropriate columns
    outbl = Table()
    outbl['type']         = [typ] * n_augmented
    outbl['spec']         = [specfile.strip()] * n_augmented
    outbl['flagtype']     = ['clean_augmented'] * n_augmented
    outbl['n_iter']       = np.zeros(n_augmented, dtype=int)
    outbl['scale_factor'] = np.zeros(n_augmented, dtype=float)
    for filte in filters:
        outbl[f"magabs_{filte}"] = np.zeros(n_augmented, dtype=float)
        outbl[f"magabs_{filte}"].format = '1.6f'

    # Generate augmentations and fill table
    idx = 0
    for it in range(number_of_iterations):
        for scale in flux_scaling_factors:
            # Scale flux and interpolate
            scaled_flam = flam * scale
            interp_lam, interp_flam, flagtype = interpspecfiletrum(
                lam, scaled_flam,
                filter_lam_min=filter_lam_min,
                filter_lam_max=filter_lam_max
            ...
            unique_idx = np.unique(interp_lam, return_index=True)[1]
            interp_lam = interp_lam[unique_idx]
            interp_flam = interp_flam[unique_idx]

            # Compute magnitudes for this augmentation
            mags = lsst.get_ab_magnitudes(interp_flam * flamunit, interp_lam * lamunit).as_array()[0]

            # Populate row idx
            outbl['n_iter'][idx]       = it
            outbl['scale_factor'][idx] = scale
            outbl['flagtype'][idx]     = f"aug_{flagtype}"
            for ff, filte in enumerate(filters):
                outbl[f"magabs_{filte}"][idx] = mags[ff]
            idx += 1

    # Write aggregated table to a single .tmp FITS file
    outbl.write(table_path, format='fits')

# ✅ 메인 루프
# for nn, specfile in enumerate(spectra):
#     print(f"[{nn+1}/{len(spectra)} ({(nn+1)/len(spectra):.1%})] {specfile}" + " " * 50, end="\r")
#     process_spectrum(nn, specfile, rep_wmetatbl, AUGMENTED_BALANCED_PHOT_RUBIN_DATA)
#%%
...
# For each spectrum, dynamically augment so that each class reaches at least n_target_per_class.
# Augmentation uses random log-uniform flux scaling.

# Determine class counts for per-class augmentation
unique_types, type_counts = np.unique(types, return_counts=True)
n_target_per_class = 30000  # or desired minimum number of augmented samples per class
...
# For each spectrum, dynamically set number of augmentations to ensure at least n_target_per_class per class
for nn, (specfile, typ) in enumerate(zip(spectra, types)):
	count = type_counts[np.where(unique_types == typ)[0][0]]
	n_target = int(np.ceil(n_target_per_class / count))
	flux_scaling_factors_this = sample_flux_scaling_factors(n_target, low=0.1, high=10.0, seed=nn+42)
	flux_scaling_factors_this = np.append(flux_scaling_factors_this, 1.0) # original one
	print(f"[{nn+1:0>4}/{len(spectra):0>4} ({(nn+1)/len(spectra):.1%})] {specfile} | {typ} | {n_target} augmentations", end="\r")
	process_spectrum_augmentation(
		nn, specfile, typ, AUGMENTED_BALANCED_PHOT_RUBIN_DATA,
		number_of_iterations=1,
		flux_scaling_factors=flux_scaling_factors_this
	)
	# break

# %%
tmptables = sorted(glob.glob(f"{AUGMENTED_BALANCED_PHOT_RUBIN_DATA}/*.tmp"))
...
synphot_normal_class_output_table = os.path.join(AUGMENTED_BALANCED_PHOT_RUBIN_DATA, "synphot_normal_class.csv")	
...
plt.tight_layout()
plotname = os.path.join(AUGMENTED_BALANCED_PHOT_RUBIN_DATA, "color-color.png")
...
logtxt += "END\n"
logtxt_path = os.path.join(AUGMENTED_BALANCED_PHOT_RUBIN_DATA, "log.txt")
