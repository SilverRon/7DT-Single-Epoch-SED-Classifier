...
#     process_spectrum(nn, specfile, rep_wmetatbl, PHOT_RUBIN_DATA)
...
spectra = metatbl['path_spectra_file'].value
types = metatbl['broad_type'].value
# %%
...
for nn, (specfile, typ) in enumerate(zip(spectra, types)):
	...
	process_spectrum(nn, specfile, typ, PHOT_RUBIN_DATA)

tmptables = sorted(glob.glob(f"{PHOT_RUBIN_DATA}/*.tmp"))
...
synphot_normal_class_output_table = os.path.join(PHOT_RUBIN_DATA, "synphot_normal_class.csv")	
...
logtxt += "END\n"
logtxt_path = os.path.join(PHOT_RUBIN_DATA)
