...
flux_scaling_factors = [1e-1, 1e0, 1e1]  # flux scale factors for augmentation
number_of_iterations = 5                  # number of augmentation iterations per factor
...
#     process_spectrum(nn, specfile, rep_wmetatbl, AUGMNETED_PHOT_RUBIN_DATA)
#%%
...
for nn, (specfile, typ) in enumerate(zip(spectra, types)):
	...
	process_spectrum_augmentation(nn, specfile, typ, AUGMNETED_PHOT_RUBIN_DATA, number_of_iterations, flux_scaling_factors)
	# break

# %%
tmptables = sorted(glob.glob(f"{AUGMNETED_PHOT_RUBIN_DATA}/*.tmp"))
...
synphot_normal_class_output_table = os.path.join(AUGMNETED_PHOT_RUBIN_DATA, "synphot_normal_class.csv")	
...
logtxt += "END\n"
logtxt_path = os.path.join(AUGMNETED_PHOT_RUBIN_DATA, "log.txt")
