...
def synth_phot(wave, flux, wave_lvf, resp_lvf, tol=1e-3, return_photonrate = False):
    ...
    flux_resamp = interp_numba(wave_resamp, wave, flux)
    resp_resamp = interp_numba(wave_resamp, wave_lvf, resp_lvf)
...
class SevenDT:
	...
	def get_CMOS_IMX455_QE(self, path_table='../simulator/QE.csv'):
	...
	def get_CMOS_IMX455_QE_forum(self, path_table='../simulator/sony.imx455.qhy600.dat'):
		T_qe = ascii.read(path_table)
	...
	def get_optics(self, path_table='../simulator/optics.efficiency.dr350.dr500.csv'):
	...
	def get_sky(self, path_table='../simulator/skytable.fits'):
	...
	def plot_point_source_depth_comp(self, spherex=True, ps1=True, smss=True,):
		...
		#	SPHEREx
		sphxcsvlist = sorted(glob.glob('../simulator/SPHEREx*.csv'))
		total_table_list = []
	...
	def get_sdss_filterset(self):
		#	Filter Table List
		sdsslist = sorted(glob.glob('../simulator/Chroma*eff.txt'))
